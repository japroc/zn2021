...
module Connection {
  ...
  /** Gets a reference to an instance of `db.Connection`. */
  DataFlow::Node local_instance() { local_instance(DataFlow::TypeTracker::end()).flowsTo(result) }
...
module cursor {
  ...
  private DataFlow::LocalSourceNode localMethodResult(DataFlow::TypeTracker t) {
    t.start() and
    result.asCfgNode().(CallNode).getFunction() = methodRef().asCfgNode()
    or
    exists(DataFlow::TypeTracker t2 | result = localMethodResult(t2).track(t2, t))
  }

  /** Gets a reference to a result of calling the `cursor` method on a connection. */
  DataFlow::Node localMethodResult() { localMethodResult(DataFlow::TypeTracker::end()).flowsTo(result) }

  DataFlow::Node methodResult() {
    result = localMethodResult()
    or
    same_attribute_store_read(localMethodResult().asExpr(), result.asExpr())
  }
}
